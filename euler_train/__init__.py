...
def init(
    ...
    pipeline:
        Optional pipeline identity mapping.  When the run is launched by
        an automated pipeline, pass a dict with at least ``attach_id``
        (a non-empty string) so downstream processes can correlate
        file-based artifacts back to the originating pipeline stage.
        Stored in ``meta.json`` under ``pipeline``.  Additional keys
        (e.g. ``stage``, ``pipeline_id``) are preserved as-is.
    """
    return Run(
